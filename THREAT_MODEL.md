...
- Public report JSON, D1 retention records, cache entries, and browser views must not contain bearer cookies, signed query strings, raw response bodies, or reconstructable secret material.
...
| Large response or concurrent probe abuse | 46-request hard budget (32 on MCP), six concurrent outbound connections (four on MCP), a scan-wide outbound deadline, bounded body readers, and phase partial status. MCP daily quota defaults to 10 versus 50 for the web form. Well-formed report reads have a separate daily cap; ETag revalidation is uncharged. | IP quotas are bypassable by rotation while the beta remains open |
| Offline guessing of quota identifiers after D1 disclosure | Daily quota keys use a managed-secret HMAC over the scope, date, and source IP. A missing or short HMAC key fails closed with an operator-visible error. | Operators must protect and deliberately rotate the Worker secret; rotation resets active daily counters |
| Cross-caller recent-scan cache reuse | Recent-scan cache keys include the daily quota client identity, so a stranger cannot reuse another caller's report ID or skip quota | Same-IP callers on the same UTC day can still share a 5-minute cache entry |
...
| Public report secret disclosure | Cookie values are removed, script source queries are stripped, URL-bearing header evidence is redacted, secret findings store only a SHA-256 fingerprint of the match, and reports use private no-store caching | Existing reports created before this release require retention expiry or manual review |
